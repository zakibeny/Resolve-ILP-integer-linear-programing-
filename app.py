...
def uflp_to_ilp(f, c):
    """
    تحويل بيانات UFLP (f: تكاليف الفتح، c: تكاليف النقل) إلى صيغة ILP:
    المتغيرات: y (n) ثنائية، x (n*m) مستمرة.
    الهدف: f·y + c_flatten·x
    القيود:
        - لكل عميل j: sum_i x_ij = 1
        - لكل i,j: x_ij ≤ y_i
    """
    n = len(f)
    m = c.shape[1]
...
def evaluate_solution(x, obj, A, b, sense, uflp_info=None):
    """
    تقييم حل ILP (x متجه صحيح). إذا كانت المسألة محولة من UFLP، نستخدم التقييم الأصلي.
    """
    if uflp_info is not None:
        # تقييم سريع باستخدام طريقة UFLP
        n_y = uflp_info['n_y']
        f = uflp_info['f']
        c = uflp_info['c']
...
def local_search(x, best_cost, obj, A, b, sense=1, uflp_info=None):
    ...
    best_x = x.copy()
    best = best_cost
    # بحث محلي بسيط: قلب كل متغير على حدة (للمتغيرات الثنائية)
    for i in range(n):
        x_new = best_x.copy()
        x_new[i] = 1 - x_new[i]
...
def read_uflp_file(text):
    ...
    if len(parts) != 3 or parts[2] != '0':
        # قد يكون بدون الصفر؟ نتحقق من n m فقط
        if len(parts) == 2:
            n = int(parts[0])
            m = int(parts[1])
        else:
            raise ValueError("السطر الأول غير صحيح: يجب أن يحتوي على n m أو n m 0")
    else:
        n = int(parts[0])
        m = int(parts[1])
...
# إزالة اختيار نوع المسألة لأننا سنعالج الملفات تلقائياً
# ولكن نبقيه للتوضيح البصري أو لاستخدامه في الإدخال اليدوي
        ...
        with st.spinner("Reading file and running algorithm..."):
            ...
            try:
                ...
                if len(first_parts) == 3 and first_parts[2] == '0' or (len(first_parts) == 2 and is_uflp):
                    # UFLP (قد يكون بدون الصفر لكن المستخدم اختار UFLP)
                    ...
                    n_vars = n
                    uflp_info = None
                    sense = 1  # نفترض تصغير، يمكن تعديله لاحقاً
                # تشغيل الخوارزمية
...
with tab3:
    ...
    if is_uflp:
        # إدخال UFLP يدوي مع إصلاح مشكلة الأبعاد
        # التأكد من أن مصفوفات الجلسة محدثة ومناسبة للأبعاد الحالية
        if ('f_man' not in st.session_state or 
            ...
            # حتى لو كانت المفاتيح موجودة، نتأكد أن c_man هي مصفوفة ثنائية الأبعاد بالشكل الصحيح
        ...
        # إنشاء مصفوفة مؤقتة لتخزين القيم المدخلة
        ...
        for i in range(n_man):
            ...
            for j in range(m_man):
                with cols[j % 5]:
                    # الوصول إلى القيمة الحالية بأمان
                    try:
...
if 'result' in st.session_state:
    ...
    if res['gap_history']:
        ...
        # إصلاح التحذير use_container_width
